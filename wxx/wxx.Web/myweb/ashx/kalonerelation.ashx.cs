...
namespace wxx.ashx
{
    ...
    /// 编辑说明：
    /// </summary>
    public class kalonerelation : baseashxClass
    {
        
        public override void ProcessRequest(HttpContext context)
        {
            wxx.BLL.kalonerelation bll = new wxx.BLL.kalonerelation(context); 
            ...
            {
                        if (context.Request.QueryString["comboxname"] == "new1")
                        {
            ...
            else if (context.Request.QueryString["type"] == "save")//保存修改或添加 
            {
                    ...
                    string editTime = context.Request.QueryString["editTime"];
                ...
                {
                    bll.Update(context.Request.QueryString["Id"].ToString(),userName,userID,category,level,editTime,new1,new2);
                }
                else
                {
                    bll.Add(userName,userID,category,level,editTime,new1,new2);
                }
            ...
            else if (Convert.ToString(context.Request.Form["action"]) == "query")
            {
                ...
                string strret = bll.GetListByPageColumns_tojson("Id,userName,userID,category,level,editTime,new1,new2", pagestrif, "Id");
                context.Response.Write(strret);
                return;
            }
            else
