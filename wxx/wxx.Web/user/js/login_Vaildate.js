...
(function($,w){
    var errorMessage = {
            ...
            phonePageIsRegister : '该手机号码已经注册！您可以<a href="./login.html">立即登录</a>',
            ...
            captchaError : '验证码错误！',
            rPassEmpty : '注册密码不能为空！',
            rPassError : '密码格式错误。请使用6-10位数字和字母组合',
            passAgain : '二次密码输入不一致',
            agree : '请同意外卖超人"注册条款"',
            ...
            if(isClearData){
                for(item in $loginObj){
                    if(item == 'remember'){
                        ...
                        $loginObj[item].addClass('loginChecked').removeClass('lRemember');
                        continue;
                    }
                    $loginObj[item].val('');
                }
                for(item in $registerObj){
                    if(item == 'agree'){
                        $registerObj[item].addClass('loginChecked').removeClass('lRemember');
                        continue;
                    }
        ...
        showErrorMessage = function(obj,message){
            obj.parent().parent().addClass("form-error-bug");
            obj.parent().addClass("form-error").next().html(message);
            return false;
        },
        hideErrorMessage = function(obj){
            obj.parent().parent().removeClass("form-error-bug");
        ...
        vaildateClientRegisterData = function(obj){
            ...
            }else if(!reg.passWord.test(ovs.passVal)){
            ...
            if(!obj.agree.hasClass("loginChecked")){
        ...
        loginFunc = function($this,func){
            if(vaildateClientLoginData($loginObj)){
                ...
                $.ajax({
                    url: userVaildate,
                    type:'POST',
                    data:{'type':'login','Account':$loginObj.phone.val(),'PassWord':$loginObj.pass.val()},
                    //$loginObj.remember.hasClass('loginChecked') ? 1 : 0 },
                    beforeSend:function(){
            ...
            if(d.status == "ok" ){
                ...
                switch(d.failed_code){
                    ...
                    case '1025':
                        ...
                        break;
                    case '3006':
                        showErrorMessage($registerObj["pass"],d.failed_msg);
                        break;
                    default:
        ...
        registerFunc = function($this,func){
            if(vaildateClientRegisterData($registerObj)){
                ...
                function error(d) {
                    commitBtnStyle($this, false);
                    if(d.failed_code == '3006'){
                        showErrorMessage($registerObj.pass, d && d.failed_msg);
                    }else{
    ...
    $("#loginTab").on("click","span",function(){
        ...
        if($(this).hasClass("login-btn")){
            $("#loginPageBox").show();
            $("#registerPageBox").hide();
        }else{
            $("#loginPageBox").hide();
            $("#registerPageBox").show();
        }
    });
    ...
    function redirectUrl(){
        ...
        if(oSearch.redirect_url){
            var url = decodeURIComponent(oSearch.redirect_url);
    ...
    $("#registerPageBtn").on('click',function(){
        var $this = $(this);
        $("#registerPageBox").find(".form-error-bug").removeClass("form-error-bug");
    ...
    $("#lRemember,#rAgree").on('click',function(){
        //模拟input点击事件 状态切换
        if($(this).hasClass('loginChecked')){
            $(this).addClass("lRemember").removeClass('loginChecked');
        }else{
            $(this).addClass("loginChecked").removeClass('lRemember');
        }
    });

    if( $obj.registerBtn[0]){
        agreeClickFunc = function (){
            $registerObj.agree.hasClass("loginChecked") ? $obj.registerBtn.removeAttr("disabled") : $obj.registerBtn.attr("disabled","disabled");
        }
    }else{
        agreeClickFunc = function (){
            $registerObj.agree.hasClass("loginChecked") ? $("#registerPageBtn").removeAttr("disabled") : $("#registerPageBtn").attr("disabled","disabled");
        }
    }
    ...
    function customPost(url, data, success, error) {
        $.ajax({
			...
			//error: function(data) { if ($.isFunction(success)) success(data)},
    ...
    if(!placeholderSupport()){
        var target = $("input[placeholder]").not(":password");
