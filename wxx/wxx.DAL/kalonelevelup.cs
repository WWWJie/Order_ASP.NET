...
namespace wxx.DAL
{
	...
	public partial class kalonelevelup
	{
		...
		/// 增加一条数据
		/// </summary>
        public int Add(wxx.Model.kalonelevelup model)
		{
			StringBuilder strSql=new StringBuilder();
         
			strSql.Append("insert into kalonelevelup(");
			strSql.Append("userName,userID,upgradeCategory,upgradeLevel,new1,new2)  values (");
			strSql.Append("@userName,@userID,@upgradeCategory,@upgradeLevel,@new1,@new2)");
			strSql.Append(";");
			MySqlParameter[] parameters = {
					new MySqlParameter("@userName", MySqlDbType.VarChar,255),
					new MySqlParameter("@userID", MySqlDbType.VarChar,255),
					new MySqlParameter("@upgradeCategory", MySqlDbType.VarChar,255),
					new MySqlParameter("@upgradeLevel", MySqlDbType.VarChar,255),
					new MySqlParameter("@new1", MySqlDbType.VarChar,255),
					new MySqlParameter("@new2", MySqlDbType.VarChar,255)};
			parameters[0].Value = model.userName;
			parameters[1].Value = model.userID;
			parameters[2].Value = model.upgradeCategory;
			parameters[3].Value = model.upgradeLevel;
			parameters[4].Value = model.new1;
			parameters[5].Value = model.new2;
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(wxx.Model.kalonelevelup model)
		{
			StringBuilder strSql=new StringBuilder();

            strSql.Append("update kalonelevelup set ");
            strSql.Append("userName=@userName,");
            strSql.Append("userID=@userID,");
            strSql.Append("upgradeCategory=@upgradeCategory,");
            strSql.Append("upgradeLevel=@upgradeLevel,");
		...
		public bool Delete(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from kalonelevelup ");
            strSql.Append(" where Id=@Id");
            MySqlParameter[] parameters = {
		...
		/// 得到一个对象实体
		/// </summary>
        public wxx.Model.kalonelevelup GetModel(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select   * from kalonelevelup ");
            ...
            wxx.Model.kalonelevelup model = new wxx.Model.kalonelevelup();
		...
		/// 得到一个对象实体
		/// </summary>
		public wxx.Model.kalonelevelup DataRowToModel(DataRow row)
		{
			wxx.Model.kalonelevelup model=new wxx.Model.kalonelevelup();
			if (row != null)
			{
                ...
                if (row["upgradeCategory"] != null )
                {
                    model.upgradeCategory = row["upgradeCategory"].ToString();
                }
                if (row["upgradeLevel"] != null )
                {
                    model.upgradeLevel = row["upgradeLevel"].ToString();
                }
                if (row["new1"] != null )
                {
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM kalonelevelup ");
            if (strWhere.Trim() != "")
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM kalonelevelup ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
        ...
        private DataSet GetListByPage(string columslist, string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from kalonelevelup T ");
        ...
        {
            string str = DbHelperMySQL.GetListByPageColumns_tojson(columslist, strWhere, orderby, PageSize, PageIndex, "kalonelevelup");
        ...
        {
            string str = DbHelperMySQL.GetListByColumns_tojson(columslist, strWhere, orderby, "kalonelevelup");
        ...
        {
            string strsql = DbHelperMySQL.GetListByPageColumns(columslist, strWhere, orderby, PageSize, PageIndex, "kalonelevelup");
